...
4. **Architecture simplified**: Collapse file count and 6 phases → 3; remove `CenterLabelView.swift` and `dialAngle` from model
...
### Design Iterations (2026-03-18)

6. **Affordances overhaul**: Knob always visible at 12 o'clock when idle; "Drag to set" hint text; tick marks on the track at 5-min intervals; knob grows during drag (18→22px); pointing hand cursor on hover
7. **Snapping removed entirely**: Smooth dragging with duration rounded to whole minutes on drag release only. Center label and end time also display rounded values during drag.
8. **Multi-revolution arc coloring**: Each full revolution draws as a layer — accent (0–1hr) → red (1–2hr) → deeper red-purple (2–3hr) → deep purple (3–4hr+). Knob color matches current revolution.
9. **Menubar format corrected**: `H:MM:SS` for >1hr (not `H:MM`), wider status item (72px)
10. **Center label "Xh Ym" during drag**: Shows immediately while setting, not only when timer is running
11. **Dial adjustment fix**: `arcAngle` for running state uses `(remaining/3600)*2π` directly instead of broken `remaining/totalDuration * cumulativeAngle` ratio; syncs `cumulativeAngle` on drag start
12. **Flash sequence redesigned**: Text color alternation (red text ↔ white-on-red background) at 300ms intervals for 3 seconds — replaces icon flash which looked like a bug
13. **Popover state transitions**: Only fire on actual state transitions via `lastObservedState` tracking, not on every tick — fixes popover closing immediately when opened during running timer
14. **First mouse support**: `FirstMouseHostingView<Content>` subclass overrides `acceptsFirstMouse` so drag works immediately without click-to-focus
15. **Swift concurrency**: `@MainActor` on AppDelegate, `MainActor.assumeIsolated` in Timer closures, `@preconcurrency UNUserNotificationCenterDelegate`
